...
extension CategoryViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: K.categoryCellID, for: indexPath) as! CategoryCollectionViewCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        NetworkManager.singleton.getArticles(passedInCategory: selectedCategory.lowercased()) { result in
            ...
            case let .success(gotArticles):
                ...
                let detailedNewsVC  = sampleStoryBoard.instantiateViewController(withIdentifier: K.headlinesCellID) as! DetailedNewsVC
                detailedNewsVC.headlines = gotArticles!
                detailedNewsVC.category = self.category[indexPath.row]
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        NetworkManager.singleton.getSearchArticles(passedInQuery: properSearchQuery) { result in
            ...
            case let .success(gotArticles):
                ...
                let detailedNewsVC  = getStoryBoard.instantiateViewController(withIdentifier: K.headlinesCellID) as! DetailedNewsVC
                detailedNewsVC.headlines = gotArticles!
                if gotArticles!.isEmpty {
                    detailedNewsVC.searchQuery = "No Results for \(searchQueryText)"
                    self.navigationController?.pushViewController(detailedNewsVC, animated: true)
                    
                }else {
                    detailedNewsVC.searchQuery = "Results for \(searchQueryText)"
                    self.navigationController?.pushViewController(detailedNewsVC, animated: true)
                }
            case let .failure(gotError):
                print(gotError)
